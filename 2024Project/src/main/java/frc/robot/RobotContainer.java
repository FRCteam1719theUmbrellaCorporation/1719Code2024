  ...
  private void configureBindings() {
    ...
    new JoystickButton(m_helperController, 1).onTrue(
        new SequentialCommandGroup(
            ...
            // m_swerveDrive.isFieldOriented = !new JoystickButton(m_driverController, 3).getAsBoolean();

    new JoystickButton(m_driverController, 3).onTrue(
        new InstantCommand(() -> {
          m_swerveDrive.isFieldOriented = false;
        }));

    new JoystickButton(m_driverController, 3).onFalse(
        new InstantCommand(() -> {
          m_swerveDrive.isFieldOriented = true;
        }));
